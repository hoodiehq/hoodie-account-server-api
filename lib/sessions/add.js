...
function addSession (state, options) {
  ...
  if (options.account.username) {
    ...
    var tokenTimeout = options.account.token.timeout * 1000
    var tokenCreatedAt = options.account.token.createdAt
    if(!Date.parse(tokenCreatedAt) + tokenTimeout < Date.now()) {
